...
def trainer(args):
    ...
    if args.logs:
        ...
        binvox_saved_path = params.output_dir + '/' + args.model_name + '/' + args.logs + '/binvox'
        
        if not os.path.exists(image_saved_path):
            os.makedirs(image_saved_path)
        if not os.path.exists(model_saved_path):
            os.makedirs(model_saved_path)
        if not os.path.exists(binvox_saved_path):
            os.makedirs(binvox_saved_path)

    # datset define
    # dsets_path = args.input_dir + args.data_dir + "train/"
    # dsets_path = params.data_dir + params.model_dir + "30/train/"
    dsets_path="...3D_Datasets/3d_lattice_binvox_128/"
    
    # if params.cube_len == 64:
    #     dsets_path = params.data_dir + params.model_dir + "30/train64/"

    print("Dataset path",dsets_path)  # ../volumetric_data/chair/30/train/

    #train_dsets = ShapeNetDataset(dsets_path, args, "train")
    # val_dsets = ShapeNetDataset(dsets_path, args, "val")
    
    binvox_files=glob.glob(".../3D_Datasets/3d_lattice_binvox_128/*.binvox")
    dataset=[]
    for file in binvox_files:
        binvox_file= load_binvox_data(file)
        binvox_file= binvox_file.reshape(128,128,128)
        dataset.append(binvox_file)
        
    print(len(dataset))
    train_dsets=np.array(dataset)
    
    print(len(train_dsets))
        
    print("dataset: ", len(dataset))   
        
    train_dset_loaders = torch.utils.data.DataLoader(dataset, batch_size=params.batch_size, shuffle=True,
                                                     num_workers=1)
        ...
        for phase in ['train']:
            ...
            for i, X in enumerate(tqdm(dset_loaders[phase])):
                ...
                if args.logs:
                    ...
                    if itr_train % 20 == 0 and phase == 'train':
            ...
            D_loss.append(epoch_loss_D)
            G_loss.append(epoch_loss_adv_G)



            if (epoch) % params.model_save_step == 0:
                print('model_saved, images_saved...')
                torch.save(G.state_dict(), model_saved_path + '/G{}.pth'.format(epoch))
                torch.save(D.state_dict(), model_saved_path + '/D{}.pth'.format(epoch))
                ...
                model_path= binvox_saved_path +"/{:03d}.binvox".format(epoch)
                
                write_binvox(samples[0], model_path)
                print("binvox_saved...")
    return epoch_loss_D, epoch_loss_adv_G
     
    plt.figure(figsize=(10,5))
    plt.title("Generator and Discriminator Loss During Training")
    plt.plot(d_loss.item(),label="G")
    plt.plot(adv_g_loss.item(),label="D")
